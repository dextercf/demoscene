...
def screen_tutorial():
    lines = [
        ...
        f"  {Y}Parties     {DG}-- scene events you attend and survive{RST}",
...
# Each entry: (tools, activities)
# tools    — pick exactly one at random (assembler / tracker / editor / etc.)
# activities — randomly include each at 65% chance
_PROD_STEP_POOLS = {
    "cracktro": (
        [
...
def screen_produce_animation(label, dkey, gained, failed, rival_name=None, rng=None):
    ...
    # Pick one tool, randomly sample activities
    pool_tools, pool_acts = _PROD_STEP_POOLS.get(dkey, _PROD_STEP_POOLS["4k"])
    steps = [rng.choice(pool_tools)]
    for step in pool_acts:
        if rng.random() < 0.65:
            steps.append(step)
    if len(steps) < 3:
        steps = steps + pool_acts[:4 - len(steps)]
